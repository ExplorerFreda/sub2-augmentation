...
from pos.utils import extra_language_list_ud12
...
meta_configs = dotdict.DotDict(
    {
        'lang': {
            'values': extra_language_list_ud12,
            'flag': None
        },
        'tagset_path': {
        ...
        'data_path': {
            'values': [
                '../data/universal-dependencies-1.2/*/{lang}-ud-{split}*.conllu'
            ],
            'flag': None
        },
        'learning_rate': {
...
os.system(f'mkdir -p ../result/pos_tagging/extra-1.2')
for model_name in meta_configs.model_name['values']:
    for augment in meta_configs.augment['values']:
        ...
        save_result(
            sub_result, 
            f'../result/pos_tagging/extra-1.2/' \
